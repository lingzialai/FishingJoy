 /**鱼*/
function Fish(className, speedX, speedY, translateX, translateY, rotate, swingParams, width, height, hobby,beCatchedWeight,price) {
	/**鱼的dom对象*/
	this.ele = null;
 	/**鱼是否移动标志位*/
	 this.moveFlag = true;
	/*被捕获的权重*/
	 this.beCatchedWeight = beCatchedWeight;
	 //鱼的价格
	 this.price = price;
	/**鱼的className*/
...
Fish.prototype.init = function() {
	...
	this.translateY = this.translateY + 200 * Math.random();
	//可能从左边出来
	if (Math.random() > 0.5) {
...
Fish.prototype.stopMove = function(){
	var i=0;
	var fish = this;
	var fishStruggle = setInterval(function(){
		i++;
		fish.swing();
		if(i==20){
			fish.remove();
			clearInterval(fishStruggle);
		}
	},50);
